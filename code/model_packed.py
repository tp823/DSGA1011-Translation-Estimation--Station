...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input, length):
        ...
        # Sort x
        batchSize, max_len = input.shape[0], input.shape[1]
        embedded = self.dropout(self.embedding(input))
        embedded = embedded.index_select(0, idx_sort)
        
        rnn_input = nn.utils.rnn.pack_padded_sequence(embedded, lengths, batch_first=True)
        ...
        rnn_out, hidden = self.gru(rnn_input, h0)
        # output.shape: batch x seq_len x dimLSTM(x2 if bi), hidden.shape: 1(2 if bi) x batch x dimLSTM
        if self.flg_bidirectional:
            hidden = torch.cat([hidden[0], hidden[1]], 1).view(1, batchSize, -1)
        hidden = hidden.index_select(1, idx_unsort)
        rnn_out, _ = torch.nn.utils.rnn.pad_packed_sequence(rnn_out, batch_first=True)
        rnn_out = rnn_out.index_select(0, idx_unsort)
        
        new_len = rnn_out.shape[1]
        if new_len < max_len:
            new_pad = torch.zeros((rnn_out.shape[0], (max_len - new_len), rnn_out.shape[2]), device = self.device)
            rnn_out = torch.cat((rnn_out, new_pad), 1)
        
        return rnn_out, hidden.transpose(0,1)
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs, mask_enc):
        ...
        if self.flgBahAtt:
            ...
            attn_weights = attn_weights.masked_fill(mask_enc == 0, -1e9)
...
class AttRNNseq2seq(nn.Module):
    ...
    def forward(self, data, target, data_len, target_len):
        encoder_output, encoder_hidden = self.encoder(data, data_len)
        use_teacher_forcing = True if random.random() < self.teacher_forcing_ratio else False
        decoder_output, decoder_idx, mask, att_weights = self._decode(data, encoder_hidden, encoder_output, target, use_teacher_forcing)
        return decoder_output, decoder_idx, mask, att_weights

    def _decode(self, data, encoder_hidden, encoder_output, target, use_teacher_forcing):
        batchSize, max_length = target.shape
        ...
        mask_enc = (data != PAD_IDX).unsqueeze(-2)

        decoder_output = torch.zeros(batchSize, max_length, self.vocab_size_dec).to(self.device)
        for di in range(max_length):
            decoder_output_i, decoder_hidden, att_w_i = self.decoder(decoder_input, decoder_hidden, encoder_output, mask_enc)
            if use_teacher_forcing:
    ...
    def inference(self, data, target, data_len, target_len):
        encoder_output, encoder_hidden = self.encoder(data, data_len)
        decoder_output, decoder_idx, mask, att_weights = self._decode(data, encoder_hidden, encoder_output, target, use_teacher_forcing = False)
        return decoder_output, decoder_idx, mask, att_weights
...
class trainModel(object):
    ...
    def _saveModel(self):
        torch.save(self.model.state_dict(), self.savePath + '_model.pt')
